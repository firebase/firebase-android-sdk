...
/**
 * A [GenerativeModelProvider] that delegates requests to a `defaultModel` and
 * falls back to a `fallbackModel` under specified conditions.
 *
 * This class provides a mechanism to attempt an operation with a primary model,
 * and if a precondition is not met or a [FirebaseAIException] occurs (and configured to do so),
 * it switches to a secondary fallback model.
 *
 * @param defaultModel The primary [GenerativeModelProvider] to use for generating content.
 * @param fallbackModel The secondary [GenerativeModelProvider] to use if the `defaultModel`
 *   cannot be used or encounters an exception.
 ...
 *   Defaults to `true`.
 */
internal class FallbackGenerativeModel(
  ...
  companion object {
    private val TAG = FallbackGenerativeModel::class.java.simpleName
  }
}
