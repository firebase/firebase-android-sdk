...
/**
 * An implementation of {@link AppCheckProvider} that uses reCAPTCHA for device
 * attestation.
 *
 ...
 * </ol>
 */
public class RecaptchaAppCheckProvider implements AppCheckProvider {
  ...
  @AssistedInject
  public RecaptchaAppCheckProvider(
  ...
  private Task<AppCheckToken> getToken(boolean isLimitedUseToken) {
    return getRecaptchaAttestation()
        .onSuccessTask(
            liteExecutor,
            recaptchaToken -> {
              ExchangeRecaptchaTokenRequest request =
                  new ExchangeRecaptchaTokenRequest(recaptchaToken, isLimitedUseToken);
              return Tasks.call(
                  blockingExecutor,
                  () ->
  ...
  @NonNull
  private Task<String> getRecaptchaAttestation() {
