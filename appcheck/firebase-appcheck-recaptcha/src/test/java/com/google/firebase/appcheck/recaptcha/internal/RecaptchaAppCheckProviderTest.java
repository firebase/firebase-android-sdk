  ...
  public void getToken_onSuccess_setsTaskResult() throws Exception {
    ...
    verify(mockNetworkClient)
        .exchangeAttestationForAppCheckToken(
            requestCaptor.capture(), eq(NetworkClient.RECAPTCHA), eq(mockRetryManager));
  ...
  public void getLimitedUseToken_onSuccess_setsTaskResult() throws Exception {
    ...
    verify(mockNetworkClient)
        .exchangeAttestationForAppCheckToken(
            requestCaptor.capture(), eq(NetworkClient.RECAPTCHA), eq(mockRetryManager));
    JSONObject jsonObject = new JSONObject(new String(requestCaptor.getValue(), "UTF-8"));
    assertThat(jsonObject.opt(ExchangeRecaptchaTokenRequest.LIMITED_USE_TOKEN_KEY)).isEqualTo(true);
  }
}
