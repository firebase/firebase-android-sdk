...
protobuf {
  ...
  plugins {
    register("firebaseEncoders") {
      path =
        "${project(":encoders:protoc-gen-firebase-encoders").layout.buildDirectory.get().asFile.path}/libs/protoc-gen-firebase-encoders-all.jar"
    }
  }
  generateProtoTasks {
    all().forEach { task ->
      task.dependsOn(configurations.named("protobuild"))
      task.inputs.file("code_gen_cfg.textproto")
      task.plugins { register("firebaseEncoders") { option(file("code_gen_cfg.textproto").path) } }
      task.builtins {
        findByName("java")?.let { remove(it) }
        findByName("kotlin")?.let { remove(it) }
      }
    }
  }
}
