...
class KotlinInteropDetector : Detector(), SourceCodeScanner {
  companion object Issues {
    private val IMPLEMENTATION =
      Implementation(KotlinInteropDetector::class.java, Scope.JAVA_FILE_SCOPE)
  ...
  class JavaVisitor(private val context: JavaContext) : UElementHandler() {
    ...
    private fun isPublicApi(node: UDeclaration): Boolean {
      ...
      psiPackage
        .getFiles(GlobalSearchScope.projectScope(psiPackage.project))
        .find { it.name == "package-info.java" }
        ?.let {
          if (
            it.viewProvider.contents
              .toString()
              .matches(
