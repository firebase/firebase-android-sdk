...
public class FeedbackSenderTest {
    ...
    @Lightweight
    private final Executor lightweightExecutor = TestOnlyExecutors.lite();

    @Mock
    private FirebaseAppDistributionTesterApiClient mockTesterApiClient;
    @Mock
    ...
    private static Uri uriForFilename(String filename) {
        return Uri.parse(ApplicationProvider.getApplicationContext().getFileStreamPath(filename).toURI().toString());
    }
}
