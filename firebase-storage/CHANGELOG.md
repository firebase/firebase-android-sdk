...
- [fixed] Fixed an issue where tests were depending on a deprecated feature of the test framework.
  (#6927)
...
- [changed] Migrated `firebase-storage` SDK to use standard Firebase executors. (GitHub
  [#4830](//github.com/firebase/firebase-android-sdk/pull/4830){: .external})
...
- [fixed] Fixed an issue that caused an infinite number of retries with no exponential backoff for
  `uploadChunk()`.
...
- [feature] Firebase now supports Kotlin coroutines. With this release, we added
  ...
  .external} to `firebase-storage-ktx` as a transitive dependency, which exposes the
...
- [fixed] Fixed an issue that caused the SDK to crash if the download location was deleted before
  the download completed. Instead, the download now fails.
...
- [feature] Added API support for destructuring of
  ...
  and [`ListResult`](/docs/reference/kotlin/com/google/firebase/storage/ListResult).
...
- [feature] Added `getCacheControl()`, `getContentDisposition()`, `getContentEncoding()`,
  `getContentLanguage()`, and `getContentType()` to
...
- [fixed] Fixed an encoding issue in
  ...
  that caused the API to miss entries for prefixes that contained special characters.

## Kotlin

- [feature] The beta release of a [firebase_storage_full] Android library with Kotlin extensions is
  now available. The Kotlin extensions library transitively includes the base `firebase-storage`
...
- [changed] Added validation to
  ...
  and
  [`StorageReference.getMetadata()`](</docs/reference/android/com/google/firebase/storage/StorageReference.html#getMetadata()>)
