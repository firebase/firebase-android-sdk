...
- [fixed] Fixed an issue where tests were depending on a deprecated feature of
  the test framework. (#6927)
...
- [fixed] Fixed an issue where `maxUploadRetryTimeMillis` parameter is ignored
  when uploading using `putBytes()`
...
- [feature] Added support for `onSuccessTask()` and `addOnCanceledListener()`
  ...
  and
  [`FileDownloadTask`](/docs/reference/android/com/google/firebase/storage/FileDownloadTask).
