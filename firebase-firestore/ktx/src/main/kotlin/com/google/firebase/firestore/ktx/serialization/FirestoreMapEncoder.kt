...
class FirestoreMapEncoder(
    ...
    override fun beginStructure(descriptor: SerialDescriptor): CompositeEncoder {
        ...
        if (depth == 0) {
            return FirestoreMapEncoder(map, depth + 1, listOfElementsToBeEncoded)
        }
        when (descriptor.kind) {
            StructureKind.CLASS -> {
                val nextDepth = depth + 1
                map[nextDepth] = mutableMapOf()
                val innerMapKey = propertyNamesToBeEncoded[elementIndex++]
                map.getValue(depth).put(innerMapKey, map[nextDepth])
                return FirestoreMapEncoder(map, nextDepth, listOfElementsToBeEncoded)
            }
            StructureKind.LIST -> {
                val emptyList = mutableListOf<Any?>()
                val innerListKey = propertyNamesToBeEncoded[elementIndex++]
    ...
    override fun beginStructure(descriptor: SerialDescriptor): CompositeEncoder {
        when (descriptor.kind) {
            StructureKind.CLASS -> {
                ...
                return FirestoreMapEncoder(map, nextDepth, descriptor.elementNames.toMutableList())
            }
            else -> {
                throw IllegalArgumentException(
                    "Incorrect format of nested object provided: <$descriptor.kind>"
