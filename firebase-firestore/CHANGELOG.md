...
- [changed] Increased the default gRPC flow control window size from 64KB to
  256KB to speed up large document reads, and added support for configuring this
  window size via
...
- [feature] Added support for `ltrim`, `rtrim`, `stringIndexOf`, `stringRepeat`,
  `stringReplaceOne`, and `stringReplaceAll` Pipeline expressions.
...
- [fixed] Further improved performance of UTF-8 string ordering logic, which had
  degraded in v25.1.2 and received some improvements in v25.1.3.
...
- [changed] Added Kotlin extensions (KTX) APIs from
  ...
  `com.google.firebase.firestore` package. For details, see the
  [FAQ about this initiative](https://firebase.google.com/docs/android/kotlin-migration)
...
- [feature] Implemented an optimization in the local cache synchronization logic
  ...
  on the server while the client was not actively listening to the query (e.g.
  while the client was offline).
  (GitHub [#4982](//github.com/firebase/firebase-android-sdk/pull/4982){: .external})
...
- [feature] Added beta support for indexed query execution. You can enable
  ...
  index definition exported by the [firebase_cli]. Queries against the cache are
  executed using an index once the asynchronous index generation completes.
...
- [fixed] Fixed a bug where local cache inconsistencies were unnecessarily being
  resolved, causing the `Task` objects returned from `get()` invocations to
  never complete. #2404
...
- [feature] Added support for [firestore] bundles via
  ...
  and can be used to populate the cache for [firestore] without the need to read
...
- [feature] Added
  ...
  - `Query.whereNotEqualTo()` finds documents where a specified field's value
    does not equal the specified value. Neither query operator finds documents
    where the specified field isn't present.
...
- [changed] Removed Guava dependency from the SDK. This change is the first step
  in eliminating crashes caused by apps that depend on the wrong flavor of
...
- [changed] Cloud Firestore now recovers more quickly when connections suffering
...
- [fixed] Fixed an issue where devices targeting Android API level 19 or earlier
  would crash when they were unable to connect to [firestore].
...
- [fixed] Fixed an internal assertion that was triggered when an update with a
  ...
  were pending for the same document. Refer to this
...
- [fixed] Fixed an issue that prevented schema migrations for clients with large
  offline datasets. Refer to this
...
- [feature] Added functional interface
  [`FirebaseFirestore.runBatch()`](/docs/reference/android/com/google/firebase/firestore/FirebaseFirestore#runBatch( com.google.firebase.firestore.WriteBatch.Function)),
  similar to
  [`FirebaseFirestore.runTransaction()`](</docs/reference/android/com/google/firebase/firestore/FirebaseFirestore#runTransaction(com.google.firebase.firestore.Transaction.Function%3CTResult%3E)>),
...
- [changed] Some SDK errors that represent common mistakes, like permission
  errors or missing indexes, are automatically logged as warnings in addition to
  being surfaced via the API.
...
- [feature] Added
  [`Query.whereArrayContains()`](/docs/reference/android/com/google/firebase/firestore/Query.html#whereArrayContains(com.google.firebase.firestore.FieldPath, java.lang.Object))
  query operator to find documents where an array field contains a specific
  element.
...
- [changed] The
  [`Timestamp`](/docs/reference/android/com/google/firebase/Timestamp) class now
  implements
