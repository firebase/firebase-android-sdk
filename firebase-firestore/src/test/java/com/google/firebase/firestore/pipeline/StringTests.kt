...
import com.google.firebase.firestore.pipeline.Expression.Companion.blob
...
internal class StringTests {
  ...
  fun substring_onString_maxNegativePosition_startsFromZero() {
    val expr = substring(blob("abc".toByteArray()), constant(-Long.MAX_VALUE), constant(2L))
    assertEvaluatesTo(
      evaluate(expr),
  ...
  fun substring_onString_oversizedNegativePosition_startsFromZero() {
    val expr = substring(blob("abc".toByteArray()), constant(-4L), constant(2L))
    assertEvaluatesTo(
      evaluate(expr),
      encodeValue(com.google.firebase.firestore.Blob.fromBytes("ab".toByteArray())),
  ...
  fun substring_onString_negativeLength_throws() {
    val expr = substring(blob("abc".toByteArray()), constant(1L), constant(-1L))
    assertEvaluatesToError(evaluate(expr), "substring with negative length")
  }

  @Test
  fun substring_onBytes_returnsSubstring() {
    val expr = substring(blob("abc".toByteArray()), constant(1L), constant(2L))
    assertEvaluatesTo(
      evaluate(expr),
  ...
  fun substring_onBytes_largePosition_returnsEmptyString() {
    val expr = substring(blob("abc".toByteArray()), constant(Long.MAX_VALUE), constant(3L))
    assertEvaluatesTo(
      evaluate(expr),
  ...
  fun substring_onBytes_positionOnLast_returnsLastByte() {
    val expr = substring(blob("abc".toByteArray()), constant(2L), constant(2L))
    assertEvaluatesTo(
      evaluate(expr),
  ...
  fun substring_onBytes_positionPastLast_returnsEmptyByteString() {
    val expr = substring(blob("abc".toByteArray()), constant(3L), constant(2L))
    assertEvaluatesTo(
      evaluate(expr),
      encodeValue(com.google.firebase.firestore.Blob.fromByteString(ByteString.EMPTY)),
  ...
  fun substring_onBytes_positionOnZero_startsFromZero() {
    val expr = substring(blob("abc".toByteArray()), constant(0L), constant(6L))
    assertEvaluatesTo(
      evaluate(expr),
