...
class ArrayMinimumTests {
  ...
  @Test
  fun `arrayMinimum - general cases`() {
    val testCases =
      listOf(
        TestCase(array(1, 2, 3), EvaluateResultValue(encodeValue(1)), "sorted numbers"),
        TestCase(array(3, 1, 2), EvaluateResultValue(encodeValue(1)), "unsorted numbers"),
        ...
        TestCase(array("b", "a"), EvaluateResultValue(encodeValue("a")), "strings"),
    ...
    for (testCase in testCases) {
      val expr = arrayMinimum(testCase.array)
      val result = evaluate(expr)
      assertWithMessage("arrayMinimum ${testCase.description}")
        .that(result)
