  ...
  fun `where inequality`(): Unit = runBlocking {
    ...
    val pipeline1 =
      RealtimePipelineSource(db)
        .collection("/users")
        .where(field("address.zip").gt(constant(90000L)))
    assertThat(runPipeline(db, pipeline1, flowOf(*documents.toTypedArray())).toList())
    ...
    assertThat(runPipeline(db, pipeline2, flowOf(*documents.toTypedArray())).toList())
      .containsExactly(doc2)

    val pipeline3 =
      RealtimePipelineSource(db).collection("/users").where(field("address.zip").lt(constant(0L)))
    assertThat(runPipeline(db, pipeline3, flowOf(*documents.toTypedArray())).toList()).isEmpty()

    val pipeline4 =
      RealtimePipelineSource(db)
        .collection("/users")
        .where(field("address.zip").neq(constant(10011L)))
    assertThat(runPipeline(db, pipeline4, flowOf(*documents.toTypedArray())).toList())
      .containsExactly(doc1, doc3)
  }
