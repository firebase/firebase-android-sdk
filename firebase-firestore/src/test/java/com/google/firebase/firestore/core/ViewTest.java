...
public class ViewTest {
  ...
  public void testComputesMutatedDocumentKeys() {
    ...
    MutableDocument doc3 = doc("rooms/eros/messages/2", 0, map()).setLocalMutations();
  ...
  public void testRemovesKeysFromMutatedDocumentKeysWhenNewDocDoesNotHaveChanges() {
    Query query = messageQuery().limitToFirst(2);
    MutableDocument doc1 = doc("rooms/eros/messages/0", 0, map());
    MutableDocument doc2 = doc("rooms/eros/messages/1", 0, map()).setLocalMutations();
    View view = new View(query, DocumentKey.emptyKeySet());
  ...
  public void testRaisesHasPendingWritesForPendingMutationsInInitialSnapshot() {
    Query query = messageQuery();
    MutableDocument doc1 = doc("rooms/eros/messages/1", 0, map()).setLocalMutations();
    View view = new View(query, DocumentKey.emptyKeySet());
  ...
  public void testDoesntRaiseHasPendingWritesForCommittedMutationsInInitialSnapshot() {
    Query query = messageQuery();
    MutableDocument doc1 = doc("rooms/eros/messages/1", 0, map()).setCommittedMutations();
    View view = new View(query, DocumentKey.emptyKeySet());
  ...
  public void testSuppressesWriteAcknowledgementIfWatchHasNotCaughtUp() {
    ...
    MutableDocument doc1 = doc("rooms/eros/messages/1", 1, map("time", 1)).setLocalMutations();
    MutableDocument doc1Committed =
        doc("rooms/eros/messages/1", 2, map("time", 2)).setCommittedMutations();
