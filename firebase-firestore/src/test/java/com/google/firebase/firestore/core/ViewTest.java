...
public class ViewTest {
  ...
  private static ViewChange applyChanges(View view, Document... docs) {
  ...
  public void testComputesMutatedDocumentKeys() {
    ...
    Document doc3 = doc("rooms/eros/messages/2", 0, map()).withLocalMutations();
  ...
  public void testRemovesKeysFromMutatedDocumentKeysWhenNewDocDoesNotHaveChanges() {
    Query query = messageQuery().limitToFirst(2);
    Document doc1 = doc("rooms/eros/messages/0", 0, map());
    Document doc2 = doc("rooms/eros/messages/1", 0, map()).withLocalMutations();
    View view = new View(query, DocumentKey.emptyKeySet());
  ...
  public void testRaisesHasPendingWritesForPendingMutationsInInitialSnapshot() {
    Query query = messageQuery();
    Document doc1 = doc("rooms/eros/messages/1", 0, map()).withLocalMutations();
    View view = new View(query, DocumentKey.emptyKeySet());
  ...
  public void testDoesntRaiseHasPendingWritesForCommittedMutationsInInitialSnapshot() {
    Query query = messageQuery();
    Document doc1 = doc("rooms/eros/messages/1", 0, map()).withCommittedMutations();
    View view = new View(query, DocumentKey.emptyKeySet());
  ...
  public void testSuppressesWriteAcknowledgementIfWatchHasNotCaughtUp() {
    ...
    Document doc1 = doc("rooms/eros/messages/1", 1, map("time", 1)).withLocalMutations();
    Document doc1Committed =
        doc("rooms/eros/messages/1", 2, map("time", 2)).withCommittedMutations();
    Document doc1Acknowledged = doc("rooms/eros/messages/1", 2, map("time", 2));

    Document doc2 = doc("rooms/eros/messages/2", 1, map("time", 1)).withLocalMutations();
    Document doc2Modified = doc("rooms/eros/messages/2", 2, map("time", 3)).withLocalMutations();
