...
public class ViewTest {
  ...
  private static ViewChange applyChanges(View view, MutableDocument... docs) {
  ...
  public void testFiltersDocumentsBasedOnQueryWithFilters() {
    ...
    MutableDocument doc4 = doc("rooms/eros/messages/4", 0, map()); // no sort, no match
  ...
  public void testUpdatesDocumentsBasedOnQueryWithFilters() {
    ...
    MutableDocument newDoc3 = doc("rooms/eros/messages/3", 1, map("sort", 3));
    MutableDocument newDoc4 = doc("rooms/eros/messages/4", 1, map("sort", 0));
  ...
  public void testComputesMutatedDocumentKeys() {
    ...
    MutableDocument doc3 = doc("rooms/eros/messages/2", 0, map()).setHasLocalMutations(0);
  ...
  public void testRemovesKeysFromMutatedDocumentKeysWhenNewDocDoesNotHaveChanges() {
    Query query = messageQuery().limitToFirst(2);
    MutableDocument doc1 = doc("rooms/eros/messages/0", 0, map());
    MutableDocument doc2 = doc("rooms/eros/messages/1", 0, map()).setHasLocalMutations(0);
    View view = new View(query, DocumentKey.emptyKeySet());
  ...
  public void testRaisesHasPendingWritesForPendingMutationsInInitialSnapshot() {
    Query query = messageQuery();
    MutableDocument doc1 = doc("rooms/eros/messages/1", 0, map()).setHasLocalMutations(0);
    View view = new View(query, DocumentKey.emptyKeySet());
  ...
  public void testSuppressesWriteAcknowledgementIfWatchHasNotCaughtUp() {
    ...
    MutableDocument doc1 = doc("rooms/eros/messages/1", 1, map("time", 1)).setHasLocalMutations(0);
    MutableDocument doc1Committed =
        doc("rooms/eros/messages/1", 2, map("time", 2)).setHasCommittedMutations();
    MutableDocument doc1Acknowledged = doc("rooms/eros/messages/1", 2, map("time", 2));

    MutableDocument doc2 = doc("rooms/eros/messages/2", 1, map("time", 1)).setHasLocalMutations(0);
    MutableDocument doc2Modified =
        doc("rooms/eros/messages/2", 2, map("time", 3)).setHasLocalMutations(0);
