...
final class InProcessTransport implements ServerTransport, ConnectionClientTransport {
  ...
  private final Set<InProcessStream> streams =
      Collections.newSetFromMap(new IdentityHashMap<InProcessStream, Boolean>());
  ...
  InProcessTransport(
    ...
    this(
        new InProcessSocketAddress(name),
        maxInboundMetadataSize,
        authority,
        userAgent,
        eagAttrs,
        Optional.of(serverListener),
        false);
  ...
  public synchronized Runnable start(ManagedClientTransport.Listener listener) {
    ...
    if (optionalServerListener.isPresent()) {
      ...
      io.grpc.inprocess.InProcessServer server =
          io.grpc.inprocess.InProcessServer.findServer(address);
      if (server != null) {
    ...
    return new Runnable() {
      ...
      public void run() {
        synchronized (InProcessTransport.this) {
          Attributes serverTransportAttrs =
              Attributes.newBuilder()
                  .set(Grpc.TRANSPORT_ATTR_REMOTE_ADDR, address)
                  .set(Grpc.TRANSPORT_ATTR_LOCAL_ADDR, address)
                  .build();
            ...
            Status failedStatus =
                Status.RESOURCE_EXHAUSTED.withDescription(
                    String.format(
                        ...
                        clientMaxInboundMetadataSize, metadataSize));
  ...
  /**
   * Returns a new status with the same code and description. If includeCauseWithStatus is true,
   * cause is also included.
   *
   * <p>For InProcess transport to behave in the same way as the other transports, when exchanging
   * statuses between client and server and vice versa, the cause should be excluded from the
   * status. For easier debugging, the status may be optionally included.
   */
  private static Status cleanStatus(Status status, boolean includeCauseWithStatus) {
    ...
    Status clientStatus =
        Status.fromCodeValue(status.getCode().value()).withDescription(status.getDescription());
