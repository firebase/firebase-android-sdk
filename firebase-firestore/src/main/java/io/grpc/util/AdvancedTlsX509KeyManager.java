...
public final class AdvancedTlsX509KeyManager extends X509ExtendedKeyManager {
   ...
   */
  public Closeable updateIdentityCredentialsFromFile(
      File keyFile, File certFile, long period, TimeUnit unit, ScheduledExecutorService executor)
   ...
   * @param certFile the file on disk holding the certificate chain
   */
  public void updateIdentityCredentialsFromFile(File keyFile, File certFile)
      throws IOException, GeneralSecurityException {
    UpdateResult newResult = readAndUpdate(keyFile, certFile, 0, 0);
    if (!newResult.success) {
      throw new GeneralSecurityException(
    ...
    public void run() {
      try {
        UpdateResult newResult =
            readAndUpdate(this.keyFile, this.certFile, this.currentKeyTime, this.currentCertTime);
        if (newResult.success) {
        ...
        log.log(
            Level.SEVERE,
            "Failed refreshing private key and certificate chain from files. "
                + "Using previous ones",
            e);
      }
    }
  ...
  private UpdateResult readAndUpdate(File keyFile, File certFile, long oldKeyTime, long oldCertTime)
    ...
    if (newKeyTime != oldKeyTime && newCertTime != oldCertTime) {
      ...
      try {
        PrivateKey key = io.grpc.util.CertificateUtils.getPrivateKey(keyInputStream);
