...
public final class AdvancedTlsX509TrustManager extends X509ExtendedTrustManager {
  ...
  private AdvancedTlsX509TrustManager(
      Verification verification, SslSocketAndEnginePeerVerifier socketAndEnginePeerVerifier)
  ...
  /**
   * Uses the default trust certificates stored on user's local system. After this is used,
   * functions that will provide new credential data(e.g. updateTrustCredentials(),
  ...
  private void checkTrusted(
    ...
    if (this.verification != Verification.INSECURELY_SKIP_ALL_VERIFICATION) {
      ...
      if (checkingServer) {
        String algorithm =
            this.verification == Verification.CERTIFICATE_AND_HOST_NAME_VERIFICATION ? "HTTPS" : "";
        if (sslEngine != null) {
   ...
   */
  public Closeable updateTrustCredentialsFromFile(
   ...
   * @param trustCertFile the file on disk holding the trust certificates
   */
  public void updateTrustCredentialsFromFile(File trustCertFile)
      throws IOException, GeneralSecurityException {
    long updatedTime = readAndUpdate(trustCertFile, 0);
    if (updatedTime == 0) {
      throw new GeneralSecurityException(
  ...
  public interface SslSocketAndEnginePeerVerifier {
    /**
     * Verifies the peer certificate chain. For more information, please refer to {@code
