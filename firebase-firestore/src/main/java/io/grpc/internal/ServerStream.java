...
/**
 * Extension of {@link io.grpc.internal.Stream} to support server-side termination semantics.
 *
 * <p>An implementation doesn't need to be thread-safe. All methods are expected to execute quickly.
 */
public interface ServerStream extends io.grpc.internal.Stream {

  /**
   * Writes custom metadata as headers on the response stream sent to the client. This method may
   * only be called once and cannot be called after calls to {@link Stream#writeMessage} or {@link
   * #close}.
   *
  ...
  /**
   * Closes the stream for both reading and writing. A status code of {@link io.grpc.Status.Code#OK}
   * implies normal termination of the stream. Any other value implies abnormal termination.
   *
   * <p>Attempts to read from or write to the stream after closing should be ignored by
   * implementations, and should not throw exceptions.
   *
  ...
  /**
   * Attributes describing stream. This is inherited from the transport attributes, and used as the
   * basis of {@link io.grpc.ServerCall#getAttributes}.
   *
