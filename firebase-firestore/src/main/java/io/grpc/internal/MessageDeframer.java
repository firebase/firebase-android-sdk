...
public class MessageDeframer implements Closeable, Deframer {
  ...
  public interface Listener {
     ...
     */
    void messagesAvailable(io.grpc.internal.StreamListener.MessageProducer producer);
    ...
    /**
     * Called when a {@link #deframe(io.grpc.internal.ReadableBuffer)} operation failed.
     *
  ...
  private InputStream getCompressedBody() {
    if (decompressor == Codec.Identity.NONE) {
      throw Status.INTERNAL
          .withDescription("Can't decode compressed gRPC message as compression not configured")
          .asRuntimeException();
    }
  ...
  static final class SizeEnforcingInputStream extends FilterInputStream {
    ...
    private void verifySize() {
      if (count > maxMessageSize) {
        throw Status.RESOURCE_EXHAUSTED
            .withDescription(
                String.format("Decompressed gRPC message exceeds maximum size %d", maxMessageSize))
            .asRuntimeException();
      }
    }
  }
