...
/**
 * The abstract base class for {@link io.grpc.internal.ClientStream} implementations. Extending
 * classes only need to implement {@link #transportState()} and {@link #abstractClientStreamSink()}.
 * Must only be called from the sending application thread.
 */
public abstract class AbstractClientStream extends io.grpc.internal.AbstractStream
    implements io.grpc.internal.ClientStream, io.grpc.internal.MessageFramer.Sink {
  ...
  protected interface Sink {
     ...
     */
    void writeFrame(
        @Nullable io.grpc.internal.WritableBuffer frame,
        boolean endOfStream,
        boolean flush,
        int numMessages);
     ...
     * AbstractClientStream#cancel} delegates to this method.
     */
  ...
  protected abstract static class TransportState extends AbstractStream.TransportState {
    ...
    protected void inboundHeadersReceived(Metadata headers) {
      ...
      if (fullStreamDecompression && streamEncoding != null) {
        if (streamEncoding.equalsIgnoreCase("gzip")) {
          setFullStreamDecompressor(new io.grpc.internal.GzipInflatingBuffer());
     ...
     *     may already be queued up in the deframer. If {@code false}, the listener will be notified
     *     immediately after all currently completed messages in the deframer have been delivered to
     *     the application.
     ...
     *
     * @param status the new status to set
     * @param rpcProgress RPC progress that the {@link ClientStreamListener#closed(Status,
     *     RpcProgress, Metadata)} will receive
     * @param stopDelivery if {@code true}, interrupts any further delivery of inbound messages that
     *     may already be queued up in the deframer and overrides any previously queued status. If
     *     {@code false}, the listener will be notified immediately after all currently completed
     *     messages in the deframer have been delivered to the application.
     * @param trailers new instance of {@code Trailers}, either empty or those returned by the
     *     server
     */
    public final void transportReportStatus(
  ...
  private class GetFramer implements io.grpc.internal.Framer {
    private Metadata headers;
    ...
    @Override
    public io.grpc.internal.Framer setMessageCompression(boolean enable) {
      return this;
    }
