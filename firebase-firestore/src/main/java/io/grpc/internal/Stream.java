...
public interface Stream {
   ...
   * StreamListener#messagesAvailable(StreamListener.MessageProducer)}. No additional messages will
   * be delivered. If the stream has a {@code start()} method, it must be called before requesting
   * messages.
   *
   ...
   * read by the Transport. Where possible callers should use streams that are {@link
   * io.grpc.KnownLength} to improve efficiency. This method will always return immediately and will
   * not wait for the write to complete. If the stream has a {@code start()} method, it must be
   * called before writing any messages.
   *
