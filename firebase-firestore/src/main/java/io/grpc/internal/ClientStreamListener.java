...
public interface ClientStreamListener extends StreamListener {
   ...
   * when a stream immediately terminates with an error and only {@link #closed(io.grpc.Status,
  ...
  /**
   * Called when the stream is fully closed. {@link io.grpc.Status.Code#OK} is the only status code
   * that is guaranteed to have been sent from the remote server. Any other status code may have
   * been caused by abnormal stream termination. This is guaranteed to always be the final call on a
   * listener. No further callbacks will be issued.
   *
   * <p>This method should return quickly, as the same thread may be used to process other streams.
   *
