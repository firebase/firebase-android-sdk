...
final class DelayedClientTransport implements ManagedClientTransport {
   ...
   * returned; if the transport is shutdown, then a {@link io.grpc.internal.FailingClientStream} is
   * returned.
   */
  @Override
  public final io.grpc.internal.ClientStream newStream(
      ...
      while (true) {
        synchronized (lock) {
          if (shutdownStatus != null) {
            return new io.grpc.internal.FailingClientStream(shutdownStatus, tracers);
          }
          if (lastPicker == null) {
        ...
        if (transport != null) {
          return transport.newStream(
              args.getMethodDescriptor(), args.getHeaders(), args.getCallOptions(), tracers);
        }
  ...
  /**
   * Caller must call {@code syncContext.drain()} outside of lock because this method may schedule
   * tasks on syncContext.
   */
  @GuardedBy("lock")
  ...
  public final void ping(
      final io.grpc.internal.ClientTransport.PingCallback callback, Executor executor) {
   ...
   * {@link #reprocess} is called. The delayed transport will be terminated when there is no more
   * buffered streams.
   */
  @Override
  ...
  public final void shutdownNow(Status status) {
    ...
    if (savedReportTransportTerminated != null) {
      for (PendingStream stream : savedPendingStreams) {
        Runnable runnable =
            stream.setStream(new FailingClientStream(status, RpcProgress.REFUSED, stream.tracers));
        if (runnable != null) {
  ...
  final void reprocess(@Nullable SubchannelPicker picker) {
    ...
    for (final PendingStream stream : toProcess) {
      ...
      final io.grpc.internal.ClientTransport transport =
  ...
  private class PendingStream extends io.grpc.internal.DelayedStream {
