...
final class ManagedChannelServiceConfig {
  ...
  @Nullable private final MethodInfo defaultMethodConfig;
  private final Map<String, MethodInfo> serviceMethodMap;
  private final Map<String, MethodInfo> serviceMap;
  @Nullable private final Throttle retryThrottling;
  @Nullable private final Object loadBalancingConfig;
  @Nullable private final Map<String, ?> healthCheckingConfig;
  ...
  static ManagedChannelServiceConfig fromServiceConfig(
    ...
    if (retryEnabled) {
      retryThrottling = io.grpc.internal.ServiceConfigUtil.getThrottlePolicy(serviceConfig);
    }
    Map<String, MethodInfo> serviceMethodMap = new HashMap<>();
    Map<String, MethodInfo> serviceMap = new HashMap<>();
    Map<String, ?> healthCheckingConfig =
        io.grpc.internal.ServiceConfigUtil.getHealthCheckedService(serviceConfig);
    ...
    List<Map<String, ?>> methodConfigs =
        io.grpc.internal.ServiceConfigUtil.getMethodConfigFromServiceConfig(serviceConfig);
    ...
    for (Map<String, ?> methodConfig : methodConfigs) {
      MethodInfo info =
          new MethodInfo(methodConfig, retryEnabled, maxRetryAttemptsLimit, maxHedgedAttemptsLimit);

      List<Map<String, ?>> nameList =
          io.grpc.internal.ServiceConfigUtil.getNameListFromMethodConfig(methodConfig);

      if (nameList == null || nameList.isEmpty()) {
        continue;
      }
      for (Map<String, ?> name : nameList) {
        String serviceName = io.grpc.internal.ServiceConfigUtil.getServiceFromName(name);
        String methodName = io.grpc.internal.ServiceConfigUtil.getMethodFromName(name);
   ...
   * InternalConfigSelector#KEY} is not available. Returns {@code null} if there is no method config
   * in this service config.
   */
  @Nullable
  ...
  static final class MethodInfo {
    ...
    MethodInfo(
        ...
        int maxRetryAttemptsLimit,
        int maxHedgedAttemptsLimit) {
      timeoutNanos = io.grpc.internal.ServiceConfigUtil.getTimeoutFromMethodConfig(methodConfig);
      waitForReady =
          io.grpc.internal.ServiceConfigUtil.getWaitForReadyFromMethodConfig(methodConfig);
      maxInboundMessageSize =
          io.grpc.internal.ServiceConfigUtil.getMaxResponseMessageBytesFromMethodConfig(
              methodConfig);
      ...
      maxOutboundMessageSize =
          io.grpc.internal.ServiceConfigUtil.getMaxRequestMessageBytesFromMethodConfig(
              methodConfig);
      ...
      Map<String, ?> retryPolicyMap =
          retryEnabled
              ? io.grpc.internal.ServiceConfigUtil.getRetryPolicyFromMethodConfig(methodConfig)
              : null;
      retryPolicy =
          retryPolicyMap == null ? null : retryPolicy(retryPolicyMap, maxRetryAttemptsLimit);

      Map<String, ?> hedgingPolicyMap =
          retryEnabled
              ? io.grpc.internal.ServiceConfigUtil.getHedgingPolicyFromMethodConfig(methodConfig)
              : null;
      hedgingPolicy =
          hedgingPolicyMap == null ? null : hedgingPolicy(hedgingPolicyMap, maxHedgedAttemptsLimit);
    }
    ...
    private static io.grpc.internal.RetryPolicy retryPolicy(
        Map<String, ?> retryPolicy, int maxAttemptsLimit) {
      int maxAttempts =
          checkNotNull(
              io.grpc.internal.ServiceConfigUtil.getMaxAttemptsFromRetryPolicy(retryPolicy),
      ...
      long initialBackoffNanos =
          checkNotNull(
              io.grpc.internal.ServiceConfigUtil.getInitialBackoffNanosFromRetryPolicy(retryPolicy),
      ...
      long maxBackoffNanos =
          checkNotNull(
              io.grpc.internal.ServiceConfigUtil.getMaxBackoffNanosFromRetryPolicy(retryPolicy),
              "maxBackoff cannot be empty");
      checkArgument(maxBackoffNanos > 0, "maxBackoff must be greater than 0: %s", maxBackoffNanos);

      double backoffMultiplier =
          checkNotNull(
              io.grpc.internal.ServiceConfigUtil.getBackoffMultiplierFromRetryPolicy(retryPolicy),
              "backoffMultiplier cannot be empty");
      checkArgument(
          backoffMultiplier > 0, "backoffMultiplier must be greater than 0: %s", backoffMultiplier);

      Long perAttemptRecvTimeout =
          io.grpc.internal.ServiceConfigUtil.getPerAttemptRecvTimeoutNanosFromRetryPolicy(
              retryPolicy);
      checkArgument(
      ...
      Set<Code> retryableCodes =
          io.grpc.internal.ServiceConfigUtil.getRetryableStatusCodesFromRetryPolicy(retryPolicy);
      checkArgument(
          perAttemptRecvTimeout != null || !retryableCodes.isEmpty(),
          "retryableStatusCodes cannot be empty without perAttemptRecvTimeout");

      return new io.grpc.internal.RetryPolicy(
          maxAttempts,
          initialBackoffNanos,
          maxBackoffNanos,
          backoffMultiplier,
    ...
    private static io.grpc.internal.HedgingPolicy hedgingPolicy(
        Map<String, ?> hedgingPolicy, int maxAttemptsLimit) {
      int maxAttempts =
          checkNotNull(
              io.grpc.internal.ServiceConfigUtil.getMaxAttemptsFromHedgingPolicy(hedgingPolicy),
              "maxAttempts cannot be empty");
      checkArgument(maxAttempts >= 2, "maxAttempts must be greater than 1: %s", maxAttempts);
      maxAttempts = Math.min(maxAttempts, maxAttemptsLimit);
      ...
      long hedgingDelayNanos =
          checkNotNull(
              io.grpc.internal.ServiceConfigUtil.getHedgingDelayNanosFromHedgingPolicy(
                  hedgingPolicy),
              "hedgingDelay cannot be empty");
      checkArgument(
          hedgingDelayNanos >= 0, "hedgingDelay must not be negative: %s", hedgingDelayNanos);

      return new io.grpc.internal.HedgingPolicy(
          maxAttempts,
          hedgingDelayNanos,
  ...
  static final class ServiceConfigConvertedSelector extends InternalConfigSelector {
    ...
    public Result selectConfig(PickSubchannelArgs args) {
      return Result.newBuilder().setConfig(config).build();
    }
  }
}
