...
/** A logical {@link io.grpc.internal.ClientStream} that is retriable. */
abstract class RetriableStream<ReqT> implements io.grpc.internal.ClientStream {
  @VisibleForTesting
  ...
  @Nullable private final io.grpc.internal.RetryPolicy retryPolicy;
  @Nullable private final io.grpc.internal.HedgingPolicy hedgingPolicy;
  ...
  @Nullable private final Throttle throttle;

  @GuardedBy("lock")
  private final io.grpc.internal.InsightBuilder closedSubstreamsInsight =
      new io.grpc.internal.InsightBuilder();

  private volatile State state =
      new State(
          new ArrayList<BufferEntry>(8),
  ...
  RetriableStream(
      ...
      @Nullable io.grpc.internal.RetryPolicy retryPolicy,
      @Nullable io.grpc.internal.HedgingPolicy hedgingPolicy,
   ...
   */
  abstract io.grpc.internal.ClientStream newSubstream(
      Metadata headers,
      ClientStreamTracer.Factory tracerFactory,
  ...
  @Override
  public void appendTimeoutInsight(io.grpc.internal.InsightBuilder insight) {
    State currentState;
    synchronized (lock) {
    ...
    if (currentState.winningSubstream != null) {
      ...
      io.grpc.internal.InsightBuilder substreamInsight = new io.grpc.internal.InsightBuilder();
      ...
      io.grpc.internal.InsightBuilder openSubstreamsInsight = new io.grpc.internal.InsightBuilder();
      ...
      for (Substream sub : currentState.drainedSubstreams) {
        io.grpc.internal.InsightBuilder substreamInsight = new InsightBuilder();
          ...
          if (isHedging) {
            ...
            if (retryPlan.shouldRetry) {
              ...
              scheduledRetryCopy.setFuture(
                  scheduledExecutorService.schedule(
                      new RetryBackoffRunnable(), retryPlan.backoffNanos, TimeUnit.NANOSECONDS));
              return;
            }
          }
        }
  ...
  class BufferSizeTracer extends ClientStreamTracer {
    ...
    /**
     * A message is sent to the wire, so its reference would be released if no retry or hedging were
     * involved. So at this point we have to hold the reference of the message longer for retry, and
     * we need to increment {@code substream.bufferNeeded}.
     */
    @Override
  ...
  static final class Throttle {
    ...
    /**
     * 1000 times the maxTokens field of the retryThrottling policy in service config. The number of
     * tokens starts at maxTokens. The token_count will always be between 0 and maxTokens.
     */
    final int maxTokens;
