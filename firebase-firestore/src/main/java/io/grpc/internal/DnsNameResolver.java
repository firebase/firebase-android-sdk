...
public class DnsNameResolver extends NameResolver {
  ...
  @VisibleForTesting static final long DEFAULT_NETWORK_CACHE_TTL_SECONDS = 30;

  @VisibleForTesting static boolean enableJndi = Boolean.parseBoolean(JNDI_PROPERTY);

  @VisibleForTesting
  static boolean enableJndiLocalhost = Boolean.parseBoolean(JNDI_LOCALHOST_PROPERTY);

  @VisibleForTesting protected static boolean enableTxt = Boolean.parseBoolean(JNDI_TXT_PROPERTY);
  ...
  @VisibleForTesting final ProxyDetector proxyDetector;
  ...
  static List<Map<String, ?>> parseTxtResults(List<String> txtRecords) throws IOException {
    ...
    for (String txtRecord : txtRecords) {
      ...
      possibleServiceConfigChoices.addAll(io.grpc.internal.JsonUtil.checkObjectList(listChoices));
    }
  ...
  private static final Double getPercentageFromChoice(Map<String, ?> serviceConfigChoice) {
    return io.grpc.internal.JsonUtil.getNumberAsDouble(
        serviceConfigChoice, SERVICE_CONFIG_CHOICE_PERCENTAGE_KEY);
  }

  @Nullable
  private static final List<String> getClientLanguagesFromChoice(
      Map<String, ?> serviceConfigChoice) {
    return io.grpc.internal.JsonUtil.getListOfStrings(
  ...
  interface ResourceResolverFactory {
    ...
    /**
     * Creates a new resource resolver. The return value is {@code null} iff {@link
     * #unavailabilityCause()} is not null;
     */
    @Nullable
    ResourceResolver newResourceResolver();

    /**
     * Returns the reason why the resource resolver cannot be created. The return value is {@code
     * null} if {@link #newResourceResolver()} is suitable for use.
     */
    @Nullable
