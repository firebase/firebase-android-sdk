...
final class InternalSubchannel
    implements InternalInstrumented<ChannelStats>, io.grpc.internal.TransportProvider {
   ...
   * #getAddressGroups()} and {@link #toString()} access this value where they supposed to access in
   * the {@link #syncContext}. Ideally {@link Index#getAddressGroups()} can be volatile, so we don't
   * need to maintain this volatile accessor. Although, having this accessor can reduce unnecessary
   * volatile reads while it delivers clearer intention of why .
   */
  ...
  @Nullable private ScheduledHandle reconnectTask;
  @Nullable private ScheduledHandle shutdownDueToUpdateTask;
  ...
  public io.grpc.internal.ClientTransport obtainActiveTransport() {
    io.grpc.internal.ClientTransport savedTransport = activeTransport;
  ...
  private void startNewTransport() {
    ...
    String eagChannelAuthority =
        currentEagAttributes.get(EquivalentAddressGroup.ATTR_AUTHORITY_OVERRIDE);
  ...
  private void scheduleBackoff(final Status status) {
    ...
    reconnectTask =
        syncContext.schedule(
            new EndOfCurrentBackoff(), delayNanos, TimeUnit.NANOSECONDS, scheduledExecutor);
  }
  ...
  private void gotoState(final ConnectivityStateInfo newState) {
    ...
    if (state.getState() != newState.getState()) {
      Preconditions.checkState(
          state.getState() != SHUTDOWN, "Cannot transition out of SHUTDOWN to " + newState);
      state = newState;
          ...
          public void run() {
            ...
            if (savedTransport != null) {
              if (shutdownDueToUpdateTask != null) {
                // Keeping track of multiple shutdown tasks adds complexity, and shouldn't generally
                // be
        ...
        new Runnable() {
          ...
          public void run() {
            ...
            List<InternalWithLogId> transportsSnapshot =
                new ArrayList<InternalWithLogId>(transports);
          ...
          new Runnable() {
            ...
            public void run() {
              ...
              if (shutdownReason != null) {
                // activeTransport should have already been set to null by shutdown(). We keep it
                // null.
                Preconditions.checkState(
                    activeTransport == null, "Unexpected non-null activeTransport");
  ...
  abstract static class Callback {
    /**
     * Called when the subchannel is terminated, which means it's shut down and all transports have
     * been terminated.
     */
    @ForOverride
  ...
  static final class CallTracingTransport extends ForwardingConnectionClientTransport {
    ...
    private CallTracingTransport(
        ConnectionClientTransport delegate, io.grpc.internal.CallTracer callTracer) {
    ...
    public io.grpc.internal.ClientStream newStream(
      ...
      final io.grpc.internal.ClientStream streamDelegate =
          super.newStream(method, headers, callOptions, tracers);
      return new ForwardingClientStream() {
        ...
        public void start(final ClientStreamListener listener) {
          ...
          super.start(
              new ForwardingClientStreamListener() {
                @Override
  ...
  static final class TransportLogger extends ChannelLogger {
    ...
    public void log(ChannelLogLevel level, String message) {
      io.grpc.internal.ChannelLoggerImpl.logOnly(logId, level, message);
    }

    @Override
    public void log(ChannelLogLevel level, String messageFormat, Object... args) {
      io.grpc.internal.ChannelLoggerImpl.logOnly(logId, level, messageFormat, args);
    }
  }
}
