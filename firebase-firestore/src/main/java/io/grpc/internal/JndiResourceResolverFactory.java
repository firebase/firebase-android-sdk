...
final class JndiResourceResolverFactory
    implements io.grpc.internal.DnsNameResolver.ResourceResolverFactory {

  @Nullable private static final Throwable JNDI_UNAVAILABILITY_CAUSE = initJndi();
   ...
   * using a service provider when a hostname query is present, so the {@code DnsContextFactory} may
  ...
  static final class JndiResourceResolver implements ResourceResolver {
    ...
    public List<String> resolveTxt(String serviceConfigHostname) throws NamingException {
      if (logger.isLoggable(Level.FINER)) {
        logger.log(
            ...
            new Object[] {serviceConfigHostname});
      }
    ...
    public List<SrvRecord> resolveSrv(String host) throws Exception {
      ...
      List<String> rawSrvRecords = recordFetcher.getAllRecords("SRV", "dns:///" + host);
      if (logger.isLoggable(Level.FINER)) {
