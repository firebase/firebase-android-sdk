...
final class ServerCallImpl<ReqT, RespT> extends ServerCall<ReqT, RespT> {
  ...
  @VisibleForTesting static final String TOO_MANY_RESPONSES = "Too many responses";
  @VisibleForTesting static final String MISSING_RESPONSE = "Completed without a response";
  ...
  private void sendHeadersInternal(Metadata headers) {
    ...
    if (compressor == null) {
      ...
      if (messageAcceptEncoding != null) {
        ...
        if (!io.grpc.internal.GrpcUtil.iterableContains(
            ACCEPT_ENCODING_SPLITTER.split(
                new String(messageAcceptEncoding, io.grpc.internal.GrpcUtil.US_ASCII)),
  ...
  /**
   * Close the {@link io.grpc.internal.ServerStream} because an internal error occurred. Allow the
   * application to run until completion, but silently ignore interactions with the {@link
   * ServerStream} from now on.
   */
  ...
  static final class ServerStreamListenerImpl<ReqT> implements ServerStreamListener {
    ...
    private void messagesAvailableInternal(final MessageProducer producer) {
      if (call.cancelled) {
        io.grpc.internal.GrpcUtil.closeQuietly(producer);
        return;
      }
      ...
      try {
        while ((message = producer.next()) != null) {
          try {
            ...
            io.grpc.internal.GrpcUtil.closeQuietly(message);
            throw t;
          }
          message.close();
        }
