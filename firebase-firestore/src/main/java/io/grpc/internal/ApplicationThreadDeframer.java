...
/**
 * Sits between {@link AbstractStream.TransportState} and {@link io.grpc.internal.MessageDeframer}
 * to deframe in the client thread. Calls from the transport to the deframer are wrapped into an
 * {@link InitializingMessageProducer} and given back to the transport, where they will be run on
 * the client thread. Calls from the deframer back to the transport use {@link
