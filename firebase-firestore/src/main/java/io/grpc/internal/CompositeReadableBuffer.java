...
/**
 * A {@link io.grpc.internal.ReadableBuffer} that is composed of 0 or more {@link
 * io.grpc.internal.ReadableBuffer}s. This provides a facade that allows multiple buffers to be
 * treated as one.
 *
 * <p>When a buffer is added to a composite, its life cycle is controlled by the composite. Once the
 * composite has read past the end of a given buffer, that buffer is automatically closed and
 * removed from the composite.
 */
public class CompositeReadableBuffer extends AbstractReadableBuffer {

  private final Deque<io.grpc.internal.ReadableBuffer> readableBuffers;
  private Deque<io.grpc.internal.ReadableBuffer> rewindableBuffers;
  ...
  /**
   * Adds a new {@link io.grpc.internal.ReadableBuffer} at the end of the buffer list. After a
   * buffer is added, it is expected that this {@code CompositeBuffer} has complete ownership. Any
   * attempt to modify the buffer (i.e. modifying the readable bytes) may result in corruption of
   * the internal state of this {@code CompositeBuffer}.
   */
  public void addBuffer(io.grpc.internal.ReadableBuffer buffer) {
  ...
  private void enqueueBuffer(io.grpc.internal.ReadableBuffer buffer) {
    ...
    while (!compositeBuffer.readableBuffers.isEmpty()) {
      io.grpc.internal.ReadableBuffer subBuffer = compositeBuffer.readableBuffers.remove();
  ...
  private static final NoThrowReadOperation<Void> UBYTE_OP =
      new NoThrowReadOperation<Void>() {
        @Override
        public int read(
            io.grpc.internal.ReadableBuffer buffer, int length, Void unused, int value) {
  ...
  private static final NoThrowReadOperation<Void> SKIP_OP =
      new NoThrowReadOperation<Void>() {
        @Override
        public int read(
            io.grpc.internal.ReadableBuffer buffer, int length, Void unused, int unused2) {
  ...
  private static final NoThrowReadOperation<byte[]> BYTE_ARRAY_OP =
      new NoThrowReadOperation<byte[]>() {
        @Override
        public int read(
            io.grpc.internal.ReadableBuffer buffer, int length, byte[] dest, int offset) {
  ...
  private static final NoThrowReadOperation<ByteBuffer> BYTE_BUF_OP =
      new NoThrowReadOperation<ByteBuffer>() {
        @Override
        public int read(
            io.grpc.internal.ReadableBuffer buffer, int length, ByteBuffer dest, int unused) {
  ...
  private static final ReadOperation<OutputStream> STREAM_OP =
      new ReadOperation<OutputStream>() {
        @Override
        public int read(
            io.grpc.internal.ReadableBuffer buffer, int length, OutputStream dest, int unused)
            throws IOException {
  ...
  @Override
  public io.grpc.internal.ReadableBuffer readBytes(int length) {
    if (length <= 0) {
    ...
    io.grpc.internal.ReadableBuffer newBuffer = null;
    CompositeReadableBuffer newComposite = null;
    do {
      io.grpc.internal.ReadableBuffer buffer = readableBuffers.peek();
      int readable = buffer.readableBytes();
      io.grpc.internal.ReadableBuffer readBuffer;
      if (readable > length) {
  ...
  public boolean markSupported() {
    for (io.grpc.internal.ReadableBuffer buffer : readableBuffers) {
  ...
  /**
   * Executes the given {@link ReadOperation} against the {@link io.grpc.internal.ReadableBuffer}s
   * required to satisfy the requested {@code length}.
   */
  ...
  private void advanceBuffer() {
    if (marked) {
      ...
      io.grpc.internal.ReadableBuffer next = readableBuffers.peek();
      if (next != null) {
        next.mark();
      }
    } else {
  ...
  /**
   * A simple read operation to perform on a single {@link io.grpc.internal.ReadableBuffer}. All
   * state management for the buffers is done by {@link
  ...
  private interface ReadOperation<T> {
     ...
     * aggregation over the sequence of buffers in a {@link CompositeReadableBuffer}. {@code value}
     * is the return value from the prior buffer, or the "initial" value passed to {@code execute()}
     * in the case of the first buffer. {@code execute()} returns the value returned by the
     * operation called on the last buffer.
     */
    int read(io.grpc.internal.ReadableBuffer buffer, int length, T dest, int value)
        throws IOException;
  }
