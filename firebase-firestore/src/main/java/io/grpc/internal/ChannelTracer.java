...
final class ChannelTracer {
   ...
   * @param maxEvents maximum number of events that are retained in memory. If not a positive number
   *     no events will be retained, but they will still be sent to the Java logger.
  ...
  ChannelTracer(
    ...
    reportEvent(
        new ChannelTrace.Event.Builder()
            ...
            // passing the timestamp in as a parameter instead of computing it right here because
            // when
            // parent channel and subchannel both report the same event of the subchannel (e.g.
            // creation
  ...
  void updateBuilder(ChannelStats.Builder builder) {
    ...
    builder.setChannelTrace(
        new ChannelTrace.Builder()
