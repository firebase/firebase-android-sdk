...
public abstract class Http2ClientStreamTransportState extends AbstractClientStream.TransportState {
  ...
  private static final Metadata.Key<Integer> HTTP2_STATUS =
      InternalMetadata.keyOf(":status", HTTP_STATUS_MARSHALLER);
  ...
  private Status statusFromTrailers(Metadata trailers) {
    ...
    if (httpStatus != null) {
      status = io.grpc.internal.GrpcUtil.httpStatusToGrpcStatus(httpStatus);
    } else {
  ...
  private Status validateInitialMetadata(Metadata headers) {
    ...
    String contentType = headers.get(io.grpc.internal.GrpcUtil.CONTENT_TYPE_KEY);
    if (!io.grpc.internal.GrpcUtil.isGrpcContentType(contentType)) {
      return io.grpc.internal.GrpcUtil.httpStatusToGrpcStatus(httpStatus)
  ...
  /**
   * Strip HTTP transport implementation details so they don't leak via metadata into the
   * application layer.
   */
