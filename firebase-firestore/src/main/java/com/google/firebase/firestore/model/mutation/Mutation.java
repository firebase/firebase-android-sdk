...
public abstract class Mutation {
  ...
  /**
   * Builds a new mutation that would do the same as applying {@code previousMutation}, then {@code
   ...
   *
   * @param previousMutation The mutation to first apply to the document. This is either {@code
   ...
   * @param previousData The data to apply transforms on.
   * @return A map of fields to transform results.
   */
  protected Map<FieldPath, Value> localTransformResults(
      Timestamp localWriteTime, @Nullable ObjectValue previousData) {
    Map<FieldPath, Value> transformResults = new HashMap<>(fieldTransforms.size());
