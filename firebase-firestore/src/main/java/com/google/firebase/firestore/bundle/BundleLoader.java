...
public class BundleLoader {
  ...
  public HashMap<DocumentKey, Document> applyChanges() {
    ...
    HashMap<DocumentKey, Document> changes =
        bundleCallback.applyBundledDocuments(
            ImmutableHashMap.copyOf(documents), bundleMetadata.getBundleId());

    HashMap<String, HashSet<DocumentKey>> queryDocumentMap = getQueryDocumentMapping();
    for (NamedQuery namedQuery : queries) {
      HashSet<DocumentKey> documentKeys = queryDocumentMap.get(namedQuery.getName());
      bundleCallback.saveNamedQuery(namedQuery, ImmutableHashSet.adopt(documentKeys));
    }
  ...
  private HashMap<String, HashSet<DocumentKey>> getQueryDocumentMapping() {
    HashMap<String, HashSet<DocumentKey>> queryDocumentMap = new HashMap<>();
    for (NamedQuery namedQuery : queries) {
