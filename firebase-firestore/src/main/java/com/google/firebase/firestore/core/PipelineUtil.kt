...
private fun canonifyStage(stage: Stage<*>): String {
  return when (stage) {
    ...
    is LimitStage -> "${stage.name}(${stage.limit})"
      ...
      throw fail("Trying to canonify an unrecognized stage type ${stage.name}")
  }
}
...
private fun getLastEffectiveSortOrderings(pipeline: RealtimePipeline): List<Ordering> {
  ...
  HardAssert.hardFail("RealtimePipeline must contain at least one Sort stage (ensured by RewriteStages).")
}