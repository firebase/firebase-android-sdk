...
public final class RemoteSerializer {
  ...
  private Document decodeFoundDocument(BatchGetDocumentsResponse response) {
    ...
    return new Document(
        ...
        ObjectValue.valueOf(response.getFound().getFieldsMap()));
  }
      ...
      case UPDATE:
        if (mutation.hasUpdateMask()) {
          return new PatchMutation(
              ...
              ObjectValue.valueOf(mutation.getUpdate().getFieldsMap()),
  ...
  private DocumentTransform.FieldTransform encodeFieldTransform(FieldTransform fieldTransform) {
    ...
    if (transform instanceof ServerTimestampOperation) {
      ...
      arrayBuilder.addAllValues(encodeArrayTransformElements(union.getElements()));
      return DocumentTransform.FieldTransform.newBuilder()
          .setFieldPath(fieldTransform.getFieldPath().canonicalString())
      ...
      com.google.firestore.v1.ArrayValue.Builder arrayBuilder =
          com.google.firestore.v1.ArrayValue.newBuilder();
      arrayBuilder.addAllValues(encodeArrayTransformElements(remove.getElements()));
      return DocumentTransform.FieldTransform.newBuilder()
          .setFieldPath(fieldTransform.getFieldPath().canonicalString())
  ...
  private List<Value> encodeArrayTransformElements(List<FieldValue> elementsProto) {
    List<Value> result = new ArrayList<>(elementsProto.size());
    for (FieldValue element : elementsProto) {
      result.add(element.getProto());
    }
    return result;
  }
      ...
      case DOCUMENT_CHANGE:
        ...
        Document document =
            new Document(
                ...
                version,
                Document.DocumentState.SYNCED,
                ObjectValue.valueOf(docChange.getDocument().getFieldsMap()));
