...
public class CustomClassMapper {
  ...
  private static class BeanMapper<T> {
    ...
    /**
     * Returns true if a method has annotations or the Kotlin synthetic $annotations() method has
     * annotations.
     */
    private static boolean isGetterAnnotationPresent(
        Method method, Class<? extends Annotation> annotation) {
      if (method.isAnnotationPresent(annotation)) {
        return true;
      }
      // If a Kotlin property has annotations with Target=PROPERTY, a synthetic $annotations()
      // method will be generated by KAPT for this property. The annotations will actually be
      // applied on this synthetic method instead of the Java field. To capture the @{@code
      // DocumentId} and @{@code ServerTimestamp} annotations for Kotlin classes, we need to check
      // these synthetic getters here. For more information, please check this url:
      // https://github.com/JetBrains/kotlin/blob/92109a6bef9e18075996637035b63a95cf76230d/core/compiler.common.jvm/src/org/jetbrains/kotlin/load/java/JvmAbi.kt#L59
      try {
        Method getterAnnotationsMethod =
            method.getDeclaringClass().getMethod(method.getName() + "$annotations");
        return getterAnnotationsMethod.isSynthetic()
            && getterAnnotationsMethod.isAnnotationPresent(annotation);
    ...
    private void applyGetterAnnotations(Method method) {
      if (isGetterAnnotationPresent(method, ServerTimestamp.class)) {
      ...
      if (isGetterAnnotationPresent(method, DocumentId.class)) {
        Class<?> returnType = method.getReturnType();
