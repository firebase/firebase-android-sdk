...
class Field private constructor(private val fieldPath: ModelFieldPath) : Expr() {
  companion object {
...
open class FilterCondition protected constructor(name: String, vararg params: Expr) :
  Function(name, params)

open class Accumulator protected constructor(name: String, params: Array<out Expr>) :
  Function(name, params) {
  protected constructor(
    name: String,
    param: Expr?
...
class ArrayContainsAll(array: Expr, values: List<Any>) :
  FilterCondition("array_contains_all", array, ListOfExpr(toArrayOfExprOrConstant(values))) {
...
class ArrayContainsAny(array: Expr, values: List<Any>) :
  FilterCondition("array_contains_any", array, ListOfExpr(toArrayOfExprOrConstant(values))) {
  constructor(fieldName: String, values: List<Any>) : this(Field.of(fieldName), values)
}
...
class In(array: Expr, values: List<Any>) :
  FilterCondition("in", array, ListOfExpr(toArrayOfExprOrConstant(values))) {
  constructor(fieldName: String, values: List<Any>) : this(Field.of(fieldName), values)
}
...
class And(condition: Expr, vararg conditions: Expr) :
  FilterCondition("and", condition, *conditions) {
  constructor(
    condition: Expr,
    vararg conditions: Any
  ) : this(condition, *toArrayOfExprOrConstant(conditions))
...
class Xor(condition: Expr, vararg conditions: Expr) :
  FilterCondition("xor", condition, *conditions) {
  constructor(
    condition: Expr,
    vararg conditions: Any
  ) : this(condition, *toArrayOfExprOrConstant(conditions))
...
class If(condition: FilterCondition, thenExpr: Expr, elseExpr: Expr) :
...
class StrConcat internal constructor(first: Expr, vararg rest: Expr) :
  ...
  constructor(
    first: Expr,
    vararg rest: String
  ) : this(first, *rest.map(Constant::of).toTypedArray())
