...
/**
 * Implements the backend semantics for locally computed NUMERIC_MAX (maximum) transforms.
 */
public class NumericMaximumTransformOperation extends NumericTransformOperation {
  public NumericMaximumTransformOperation(Value operand) {
    super(operand);
  }
  ...
  public Value applyToLocalView(@Nullable Value previousValue, Timestamp localWriteTime) {
    ...
    if (isInteger(previousValue) && isInteger(operand)) {
      long max = Math.max(previousValue.getIntegerValue(), operandAsLong());
      return Value.newBuilder().setIntegerValue(max).build();
    } else {
      double prevDouble =
