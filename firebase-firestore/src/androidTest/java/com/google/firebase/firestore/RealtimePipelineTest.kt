...
class RealtimePipelineTest {
  ...
  fun testExpressionEvaluateToErrorInWhere() = runBlocking {
    val pipeline =
      db.realtimePipeline().collection(collRef.path).where(field("published").add(Long.MAX_VALUE).lessThan(2000))
    val options =
      RealtimePipelineOptions()
        .withMetadataChanges(MetadataChanges.INCLUDE)
        .withSource(ListenSource.CACHE)
  ...
  fun testExpressionEvaluateToErrorInSort() = runBlocking {
    val pipeline =
      db.realtimePipeline().collection(collRef.path).sort(field("published").add(Long.MAX_VALUE - 1950).descending())
    val options =
      RealtimePipelineOptions()
        .withMetadataChanges(MetadataChanges.INCLUDE)
        .withSource(ListenSource.CACHE)
