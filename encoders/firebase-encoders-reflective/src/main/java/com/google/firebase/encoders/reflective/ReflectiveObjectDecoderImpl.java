...
class ReflectiveObjectDecoderImpl<T> implements ObjectDecoder<T> {

  private final Map<String, ReflectiveDecoderFieldContext<?>> fieldContexts = new HashMap<>();

  ReflectiveObjectDecoderImpl() {}
    ...
    while (currentClass != Object.class && currentClass != null) {
      ...
      for (Method method : methods) {
        ...
        if (!fieldContexts.containsKey(fieldName)) {
          fieldContexts.put(fieldName, new ReflectiveDecoderFieldContext<>(method));
        }
      }
  ...
  private void readFields(Class<T> clazz) {
    ...
    while (currentClass != Object.class && currentClass != null) {
      for (Field field : currentClass.getDeclaredFields()) {
        ...
        if (!fieldContexts.containsKey(fieldName)) {
          fieldContexts.put(fieldName, new ReflectiveDecoderFieldContext<>(field));
        }
      }
      currentClass = currentClass.getSuperclass();
    }
