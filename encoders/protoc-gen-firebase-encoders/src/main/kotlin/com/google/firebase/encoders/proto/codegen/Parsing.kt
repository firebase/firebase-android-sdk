...
class DefaultParser @Inject constructor(private val config: CodeGenConfig) : DescriptorParser {
    ...
    private fun resolveReferences(types: Collection<UserDefined>) {
        val messageIndex = types.asSequence()
                .map { it.protobufFullName to it }
                .toMap()
        for (userDefined in types) {
            ...
            for (field in userDefined.fields) {
                (field.type as? Unresolved)?.run {
                    field.type = messageIndex[this.protobufName]
    ...
    private fun parseEnums(parent: Owner, enums: List<DescriptorProtos.EnumDescriptorProto>): Sequence<ProtoEnum> {
        return enums.asSequence().map { enum ->
            ProtoEnum(parent, enum.name, enum.valueList.map {
                ProtoEnum.Value(it.name, it.number)
            })
        }
    }

    private fun parseMessages(parent: Owner, messages: List<DescriptorProtos.DescriptorProto>): Sequence<UserDefined> {
        return messages.asSequence().flatMap { msg ->
            val m = Message(
                    owner = parent,
                    name = msg.name,
                    fields = msg.fieldList.map {
                        ProtoField(
                                name = it.name,
