...
public class MappingFileIdWriterTest {
  ...
  public void testWithoutResourceFolder() throws Exception {
    ...
    MappingFileIdWriter idWriter = new MappingFileIdWriter(resFile);
    ...
    String id = Buildtools.generateMappingFileId();
    idWriter.writeMappingFileId(id);
  ...
  @Test
  public void testIdCreateAndUpdate() throws Exception {

    File resFile = new File(new File(testResourceDirectory, "values"), "test_strings.xml");
    MappingFileIdWriter idWriter = new MappingFileIdWriter(resFile);
    MappingFileIdReader idReader = MappingFileIdReader.create(resFile);
    String id = idReader.getMappingFileId();
    Assert.assertNull(id);

    id = Buildtools.generateMappingFileId();
    idWriter.writeMappingFileId(id);
    Assert.assertEquals(id, idReader.getMappingFileId());

    String newId = Buildtools.generateMappingFileId();
    idWriter.writeMappingFileId(newId);
    Assert.assertEquals(newId, idReader.getMappingFileId());
    Assert.assertNotSame(newId, id);
  }
}
