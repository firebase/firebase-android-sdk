...
internal class SessionDataService : Service() {
  ...
  internal inner class IncomingHandler(
    ...
    fun handleForegrounding() {
      ...
      msgBundle.putString(SESSION_UPDATE_EXTRA, sessionId)
      boundClients.forEach { client ->
        Log.i(TAG, "SERVICE: Informing $client")
        val sessionUpdateMsg = Message.obtain(null, SESSION_UPDATED, 0, 0)
        sessionUpdateMsg.setData(msgBundle)
        client.send(sessionUpdateMsg)
      }
    }
