...
- [fixed] Fixed the `@Exclude` annotation doesn't been propagated to Kotlin's
  ...
  [#5626](//github.com/firebase/firebase-android-sdk/pull/5706)
...
- [unchanged] Updated to accommodate the release of the updated [database]
...
- [fixed] The SDK can now continue to issue writes for apps that send invalid
  [app_check] tokens if [app_check] enforcement is not enabled.
...
- [fixed] Fixed an issue that prevented clients from connecting to the backend
  when the app used [app_check] without [auth].
...
- [feature] Added
  ...
  and
  [`Query#get()`](</docs/reference/android/com/google/firebase/database/Query#get()>),
  which return data from the server even when older data is available in the
  local cache.
...
- [feature] The beta release of a [database] Android library with Kotlin
  ...
  [[database] KTX
  documentation](/docs/reference/kotlin/com/google/firebase/database/ktx/package-summary).

# 19.1.0

- [feature] Added support for the [firebase_database] Emulator. To connect to
  the emulator, specify "http://<hostname>:<port>/?ns=<project_id>" as your
  Database URL (via
  ...
  [database] Emulator host will be "10.0.2.2" followed by its port.
