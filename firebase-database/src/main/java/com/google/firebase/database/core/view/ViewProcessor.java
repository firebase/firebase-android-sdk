...
public class ViewProcessor {
  ...
  private ViewCache generateEventCacheAfterServerEvent(
    ...
    if (writesCache.shadowingWrite(changePath) != null) {
      ...
      if (changePath.isEmpty()) {
        ...
        hardAssert(  viewCache.getServerCache().isFullyInitialized(),"If change path is empty, we must have complete server data");
        ...
        if (childKey.isPriorityChildName()) {
          hardAssert(  changePath.size() == 1 ,"Can't have a priority with additional path components");
  ...
  private ViewCache applyServerOverwrite(
    ...
    if (changePath.isEmpty()) {
      ...
      // full update
      hardAssert(  !changePath.isEmpty() ,"An empty path should have been caught in the other branch");
      ChildKey childKey = changePath.getFront();
