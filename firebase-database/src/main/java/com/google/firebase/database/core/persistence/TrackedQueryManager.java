...
public class TrackedQueryManager {
  ...
  private void setQueryActiveFlag(QuerySpec query, boolean isActive) {
    ...
    if (trackedQuery != null) {
      ...
      hardAssert(  isActive , "If we're setting the query to inactive, we should already be tracking it!");
      trackedQuery =
  ...
  public Set<ChildKey> getKnownCompleteChildren(Path path) {
    hardAssert(  !this.isQueryComplete(QuerySpec.defaultQueryAtPath(path)) , "Path is fully complete.");
