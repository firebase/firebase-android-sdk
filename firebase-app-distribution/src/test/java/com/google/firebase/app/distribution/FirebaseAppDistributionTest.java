...
public class FirebaseAppDistributionTest {
  ...
  @Mock private TesterSignInHandler mockTesterSignInHandler;
  @Mock private CheckForNewReleaseHandler mockCheckForNewReleaseHandler;
    ...
    firebaseAppDistribution =
        spy(
            new FirebaseAppDistribution(
                firebaseApp,
                mockTesterSignInHandler,
                mockCheckForNewReleaseHandler,
                mockApkUpdater,
                mockAabUpdater,
                mockSignInStorage,
                mockLifecycleNotifier));

    when(mockTesterSignInHandler.signInTester()).thenReturn(Tasks.forResult(null));
  ...
  public void checkForNewRelease_callsSignInTester() {
    ...
    verify(mockTesterSignInHandler, times(1)).signInTester();
  }
  ...
  public void updateToNewRelease_whenNoReleaseAvailable_updateDialogNotShown() {
    when(mockSignInStorage.getSignInStatus()).thenReturn(false);
    when(mockCheckForNewReleaseHandler.checkForNewRelease()).thenReturn(Tasks.forResult(null));
  ...
  public void updateToNewRelease_whenSignInCancelled_checkForUpdateNotCalled() {
    when(mockSignInStorage.getSignInStatus()).thenReturn(false);
    when(mockTesterSignInHandler.signInTester())
        .thenReturn(
            Tasks.forException(
                new FirebaseAppDistributionException(
    ...
    verify(mockTesterSignInHandler, times(1)).signInTester();
    verify(mockCheckForNewReleaseHandler, never()).checkForNewRelease();
    assertTrue(task.getException() instanceof FirebaseAppDistributionException);
    FirebaseAppDistributionException e = (FirebaseAppDistributionException) task.getException();
