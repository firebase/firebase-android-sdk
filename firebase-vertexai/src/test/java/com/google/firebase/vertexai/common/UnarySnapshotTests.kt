...
import com.google.firebase.vertexai.type.InternalFinishReason
import com.google.firebase.vertexai.type.InternalFunctionCallPart
...
internal class UnarySnapshotTests {
  ...
  fun `short reply`() =
    goldenUnaryFile("success-basic-reply-short.json") {
      withTimeout(testTimeout) {
        ...
        response.candidates?.first()?.finishReason shouldBe InternalFinishReason.STOP
  ...
  fun `unknown enum`() =
    goldenUnaryFile("success-unknown-enum.json") {
      withTimeout(testTimeout) {
        ...
        candidate?.safetyRatings?.any { it.category == HarmCategory.InternalHarmCategory.UNKNOWN } shouldBe true
      }
    }
    ...
    goldenUnaryFile("success-including-severity.json") {
      withTimeout(testTimeout) {
        ...
        response.candidates?.first()?.safetyRatings?.all {
          it.probability == HarmProbability.InternalHarmProbability.NEGLIGIBLE
        } shouldBe true
        response.candidates?.first()?.safetyRatings?.all { it.probabilityScore != null } shouldBe
          true
        response.candidates?.first()?.safetyRatings?.all {
          it.severity == HarmSeverity.InternalHarmSeverity.NEGLIGIBLE
        } shouldBe true
  ...
  fun `prompt blocked for safety`() =
    goldenUnaryFile("failure-prompt-blocked-safety.json") {
      withTimeout(testTimeout) {
        ...
        } should { it.response.promptFeedback?.blockReason shouldBe BlockReason.InternalBlockReason.SAFETY }
      }
    }
  ...
  fun `stopped for safety`() =
    goldenUnaryFile("failure-finish-reason-safety.json") {
      withTimeout(testTimeout) {
        ...
        exception.response.candidates?.first()?.finishReason shouldBe InternalFinishReason.SAFETY
      }
    }
  ...
  fun `properly translates json text`() =
    goldenUnaryFile("success-constraint-decoding-json.json") {
      withTimeout(testTimeout) {
        ...
        response.candidates?.isEmpty() shouldBe false
        with(
          response.candidates?.first()?.content?.parts?.first()?.shouldBeInstanceOf<InternalTextPart>()
        ) {
          shouldNotBeNull()
  ...
  fun `function call contains null param`() =
    goldenUnaryFile("success-function-call-null.json") {
      withTimeout(testTimeout) {
        val response = apiController.generateContent(textGenerateContentRequest("prompt"))
        val callPart = (response.candidates!!.first().content!!.parts.first() as InternalFunctionCallPart)
      ...
      withTimeout(testTimeout) {
        ...
        val callPart =
          content.let {
            ...
            it.parts.first().shouldBeInstanceOf<InternalFunctionCallPart>()
          }
  ...
  fun `function call has no arguments field`() =
    goldenUnaryFile("success-function-call-empty-arguments.json") {
      withTimeout(testTimeout) {
        ...
        val callPart = content.parts.shouldNotBeNullOrEmpty().first() as InternalFunctionCallPart
