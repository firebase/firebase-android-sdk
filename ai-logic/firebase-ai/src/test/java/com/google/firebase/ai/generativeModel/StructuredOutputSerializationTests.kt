...
class StructuredOutputSerializationTests {
  ...
  fun `serialization works the same for cloud and on-device`(): Unit = runBlocking {
    ...
    val mockCloudResponse =
      mockk<com.google.firebase.ai.type.GenerateContentResponse.Internal>(relaxed = true)
    ...
    coEvery { mockOnDeviceModel.generateContent(capture(onDeviceRequestSlot)) } returns
      mockOnDeviceResponse
    ...
    val expectedPrefix =
      "Output a JSON object that conforms to the following schema:\n$cloudSchemaJsonString"
