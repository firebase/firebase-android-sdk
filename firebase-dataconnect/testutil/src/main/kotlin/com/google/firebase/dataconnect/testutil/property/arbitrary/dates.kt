...
data class DateTestData(val date: Date, val string: String) {
  fun withMillisOffset(millisOffset: Long): DateTestData = copy(Date(date.time + millisOffset))
}
...
fun DataConnectArb.dateOffDayBoundary(): Arb<DateTestData> =
  arbitrary(edgecases = DateEdgeCases.offDayBoundary) {
    ...
    DateTestData(dateOffDayBoundary, dateAndString.string)
  }
...
object DateEdgeCases {
  ...
  val min: DateTestData
    get() = DateTestData(date = dateFromYearMonthDayUTC(MIN_YEAR, 1, 1), string = "$MIN_YEAR-01-01")

  val max: DateTestData
    get() =
      DateTestData(date = dateFromYearMonthDayUTC(MAX_YEAR, 12, 31), string = "$MAX_YEAR-12-31")
