...
fun AnyScalarArb.number(): Arb<Double> = Arb.double()
...
fun AnyScalarArb.string(): Arb<String> =
  Arb.string(minSize = 1, maxSize = 40, codepoints = codepoints)

fun AnyScalarArb.list(): Arb<List<Any?>> = arbitrary {
  val size = Arb.int(1..3).bind()
  List(size) { any().bind() }
}

fun AnyScalarArb.map(): Arb<Map<String, Any?>> = arbitrary {
  buildMap {
    val size = Arb.int(1..3).bind()
