...
@Serializable
data class createPersonVariables(val data: Person_Data) {
  constructor(
    id: String,
    name: String,
    age: Int?
  ) : this(data = Person_Data(id = id, name = name, age = age))
...
suspend fun MutationRef<createPersonVariables, createPersonData>.execute(
  id: String,
  name: String,
  age: Int?
) = execute(createPersonVariables(id = id, name = name, age = age))
