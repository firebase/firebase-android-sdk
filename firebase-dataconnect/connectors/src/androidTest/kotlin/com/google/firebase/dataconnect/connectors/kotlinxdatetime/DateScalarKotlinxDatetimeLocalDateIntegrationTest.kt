...
////////////////////////////////////////////////////////////////////////////////
package com.google.firebase.dataconnect.connectors.kotlinxdatetime

import com.google.firebase.dataconnect.DataConnectException
import com.google.firebase.dataconnect.ExperimentalFirebaseDataConnect
import com.google.firebase.dataconnect.MutationResult
import com.google.firebase.dataconnect.QueryResult
import com.google.firebase.dataconnect.connectors.kotlinxdatetime.testutil.DemoKotlinxdatetimeConnectorIntegrationTestBase
...
// ./gradlew generateDataConnectTestingSources
////////////////////////////////////////////////////////////////////////////////
class DateScalarKotlinxDatetimeLocalDateIntegrationTest :
  DemoKotlinxdatetimeConnectorIntegrationTestBase() {
  ...
  fun dateNonNullable_MutationLocalDateVariable() =
    runTest(timeout = 1.minutes) {
      checkAll(propTestConfig, Arb.dataConnect.dateTestData()) { testData ->
        val insertResult =
          connector.dateNonNullableInsert.execute(testData.date.toKotlinxLocalDate())
        val returnedString =
  ...
  fun dateNonNullable_MutationStringVariable() =
    runTest(timeout = 1.minutes) {
      checkAll(propTestConfig, Arb.dataConnect.dateTestData()) { testData ->
        ...
        queryResult.data.item?.value shouldBe testData.date.toKotlinxLocalDate()
      }
    }
  ...
  fun dateNonNullable_QueryLocalDateVariable() =
    dateNonNullable_QueryVariable { tag, dateTestData ->
      connector.dateNonNullableGetAllByTagAndValue.execute(
        tag = tag,
        dateTestData.date.toKotlinxLocalDate()
      )
    }
  ...
  fun dateNonNullable_Update() =
    runTest(timeout = 1.minutes) {
      checkAll(
        propTestConfig,
        Arb.dataConnect.localDate().map { it.toKotlinxLocalDate() },
        Arb.dataConnect.localDate().map { it.toKotlinxLocalDate() }
      ) { date1, date2 ->
  ...
  fun dateNonNullable_UpdateToNullShouldFail() =
    runTest(timeout = 1.minutes) {
      checkAll(propTestConfig, Arb.dataConnect.localDate().map { it.toKotlinxLocalDate() }) { date
        ->
      ...
      checkAll(
        ...
        val updateResult =
          connector.dateNonNullableUpdateByTagAndValue.execute(tag) {
            value = selectedDate.date.toKotlinxLocalDate()
            newValue = date2
          }
  ...
  fun dateNonNullable_UpdateManyOmittedNewValueShouldNotChangeAny() =
    runTest(timeout = 1.minutes) {
      checkAll(
        ...
        val insertResult = connector.dateNonNullableInsert3.execute(tag, testDatas)
        val selectedDate = testDatas.selected!!.date.toKotlinxLocalDate()
        val updateResult =
  ...
  fun dateNullable_MutationLocalDateVariable() =
    runTest(timeout = 1.minutes) {
      ...
      checkAll(propTestConfig, localDates) { testData ->
        val insertResult =
          connector.dateNullableInsert.execute { value = testData.ref?.date?.toKotlinxLocalDate() }
        val returnedString =
  ...
  fun dateNullable_MutationStringVariable() =
    runTest(timeout = 1.minutes) {
      ...
      checkAll(propTestConfig, localDates) { testData ->
        ...
        queryResult.data.item?.value shouldBe testData.ref?.date?.toKotlinxLocalDate()
      }
    }
  ...
  fun dateNullable_QueryLocalDateVariable() = dateNullable_QueryVariable { tag, dateTestData ->
    connector.dateNullableGetAllByTagAndValue.execute(tag = tag) {
      value = dateTestData?.date?.toKotlinxLocalDate()
    }
  }
  ...
  private suspend fun DemoKotlinxdatetimeConnector.requestTime(): kotlinx.datetime.LocalDate {
