...
class CrashlyticsRemoteConfigListener(private val userMetadata: UserMetadata) :
  ...
  override fun onRolloutsStateChanged(rolloutsState: RolloutsState) {
    val rolloutAssignmentList = rolloutsState.rolloutAssignments

    val crashlyticsRolloutAssignmentList = mutableListOf<RolloutAssignment>()

    rolloutAssignmentList.map { assignment ->
      val crashlyticsRolloutAssignment =
        RolloutAssignment.create(
          assignment.rolloutId,
          assignment.parameterKey,
