...
public class FirebaseCrashlytics {
            ...
            Logger.getLogger()
                    .d(
                            String.format(
                                    ...
                                    buildIdInfo.getLibraryName(),
                                    buildIdInfo.getArch(),
                                    buildIdInfo.getBuildId()));
        }
                        ...
                        new Continuation<Void, Object>() {
                            ...
                            public Object then(@NonNull Task<Void> task) throws Exception {
                                if (!task.isSuccessful()) {
                                    Logger.getLogger()
                                            .e("Error fetching settings.", task.getException());
                                }
                                return null;
                            }
                        });
     ...
     *
     * <p>The default {@link FirebaseApp} instance must be initialized before this function is
     * called. See <a
     ...
     *
     * <p>The value of any key at the time of a fatal, non-fatal, or ANR event is associated with
     * that event.
     *
     ...
     *
     * <p>The value of any key at the time of a fatal, non-fatal, or ANR event will be associated
     * with that event.
     *
     * <p>Keys and associated values are visible in the session view on the Firebase Crashlytics
     * console.
     *
     * <p>A maximum of 64 key/value pairs can be written, and new keys added beyond that limit will
     * be ignored. Keys or values that exceed 1024 characters will be truncated.
     *
     * @param key A unique key
     * @param value A value to be associated with the given key
     */
    ...
    /**
     * Sets multiple custom keys and values that are associated with subsequent fatal, non-fatal,
     * and ANR reports. This method is intended as an alternative to {@code setCustomKey} in order
     * to reduce the computational load of writing out multiple key/value pairs at the same time.
     *
     * <p>Multiple calls to this method with the same key update the value for that key.
     *
     * <p>The value of any key at the time of a fatal, non-fatal, or ANR event is associated with
     ...
     * needed to get to 64). Which keys are logged versus dropped is unpredictable as there is no
    ...
    /**
     * Checks a device for any fatal crash, non-fatal error, or ANR reports that haven't yet been
     * sent to Crashlytics. If automatic data collection is enabled, then reports are uploaded
     ...
     * to send to Crashlytics. Otherwise, this method is a no-op.
     */
    ...
    /**
     * If automatic data collection is disabled, this method queues up all the reports on a device
     * for deletion. Otherwise, this method is a no-op.
     */
     ...
     * disabled. Use {@link #deleteUnsentReports()} to delete any reports stored on the device
     * without sending them to Crashlytics.
     *
     * @param enabled whether to enable automatic data collection. When set to {@code false}, the
     *     new value does not apply until the next run of the app. To disable data collection by
     *     default for all app runs, add the {@code firebase_crashlytics_collection_enabled} flag to
     ...
     *     null}, the override is cleared and automatic data collection settings are determined by
     *     the configuration in your AndroidManifest.xml or other Firebase-wide settings.
     */
