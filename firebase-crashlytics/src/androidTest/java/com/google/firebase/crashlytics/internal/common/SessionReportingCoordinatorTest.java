...
public class SessionReportingCoordinatorTest {
  ...
  public void onReportSend_successfulReportsAreDeleted() {
    ...
    when(reportSender.enqueueReport(mockReport1)).thenReturn(successfulTask);
    when(reportSender.enqueueReport(mockReport2)).thenReturn(failedTask);

    reportingCoordinator.sendReports(Runnable::run);

    verify(reportSender).enqueueReport(mockReport1);
    verify(reportSender).enqueueReport(mockReport2);
  }
