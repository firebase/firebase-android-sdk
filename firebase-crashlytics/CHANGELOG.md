...
- [feature] Added OOM and Anomaly trigger collection for the ProfilingManager
  API [#8343]

# 20.0.6

- [fixed] Fixed race condition that caused logs from background threads to not
  be attached to reports in some cases [#8034]
- [changed] Updated `firebase-sessions` dependency to v3.0.6

# 20.0.5

- [fixed] Fixed a runtime crash that could occur in minified native apps when
  using the Crashlytics C++ header (#7906)
...
- [changed] Internal changes to read version control info more efficiently
  [#6754]
- [fixed] Fixed NoSuchMethodError when getting process info on Android 13 on
  some devices [#6720]
...
- [feature] Added an overload for `recordException` that allows logging
  additional custom keys to the non fatal event [#3551]
...
- [fixed] Fixed inefficiency in the Kotlin `FirebaseCrashlytics.setCustomKeys`
  extension.
...
- [fixed] Fixed exception propagation in the case of no default uncaught
  exception handler.
...
- [feature] Expanded `firebase-sessions` library integration to work with NDK
  crashes and ANRs.
...
- [feature] Integrated with Firebase sessions library to enable upcoming
  features related to session-based crash metrics.
...
- [fixed] Fix an uncaught IllegalStateExeception that could be thrown if
  ...
  state rather than crashing. (#2504)
...
- [changed] Removed OkHttp dependency to eliminate conflicts with apps and SDKs
...
- [changed] Improved compatibility with Google Analytics. For best performance,
  make sure you're using the latest versions of the [firebase_crashlytics] SDK
  and the Firebase SDK for Google Analytics.
- [changed] Updated remaining [crashlytics] backend API calls to prepare for
  Fabric sunset.
...
- [removed] Removed the Fabric [crashlytics] API key. Now, [crashlytics] will
  always use the `google-services.json` file to associate your app with your
  Firebase project. If you linked your app from Fabric, remove the Fabric API
  key from your `AndroidManifest.xml` file.
